# ffmpeg 自动打包功能总结

> **更新日期**: 2025-12-09  
> **状态**: ✅ 已实现并测试

## 🎯 问题描述

在 Windows 应用打包时，用户需要：
1. 手动安装 ffmpeg 到系统
2. 确保 ffmpeg 在 PATH 中
3. 才能成功构建应用

这对不熟悉命令行的用户来说很困难。

## ✨ 解决方案

实现了**自动下载和打包 ffmpeg** 的功能！

### 核心特性

1. **自动下载**: 构建时自动下载对应平台的 ffmpeg
2. **智能缓存**: 避免重复下载，提高构建速度
3. **跨平台支持**: Windows, macOS, Linux
4. **零配置**: 开箱即用，无需手动安装

## 📁 新增文件

### 1. `src-python/download_ffmpeg.py`
ffmpeg 自动下载模块

**功能**:
- 从官方源下载 ffmpeg 二进制文件
- Windows: https://www.gyan.dev/ffmpeg/builds/
- Linux: https://johnvansickle.com/ffmpeg/
- macOS: 使用系统安装的 ffmpeg
- 智能缓存到 `.ffmpeg_cache/` 目录
- 支持单独运行和作为模块导入

**使用方法**:
```bash
# 下载当前平台的 ffmpeg
python download_ffmpeg.py

# 下载 Windows 平台的 ffmpeg
python download_ffmpeg.py --platform windows

# 下载 Linux 平台的 ffmpeg  
python download_ffmpeg.py --platform linux
```

### 2. `src-python/test_ffmpeg_download.py`
测试脚本

**功能**:
- 测试当前平台 ffmpeg 获取
- 测试跨平台下载
- 显示缓存状态和文件大小

**使用方法**:
```bash
python test_ffmpeg_download.py
```

### 3. `WINDOWS_BUILD_GUIDE.md`
Windows 构建详细指南

**内容**:
- 自动下载 ffmpeg 的工作原理
- 三种构建方式
- 文件结构说明
- 故障排除
- PyInstaller 跨平台限制说明

### 4. `QUICK_START_CN.md`
中文快速开始指南

**内容**:
- 快速构建步骤
- 测试方法
- 常见问题解答
- 下一步操作

## 🔧 修改的文件

### 1. `src-python/build_sidecar.py`

**主要改动**:

1. **导入自动下载模块**:
```python
from download_ffmpeg import get_ffmpeg_path as auto_get_ffmpeg
```

2. **增强的 `get_ffmpeg_path()` 函数**:
```python
def get_ffmpeg_path(target_platform=None):
    # 首先尝试自动下载
    if auto_get_ffmpeg:
        try:
            return auto_get_ffmpeg(target_platform)
        except Exception as e:
            # 回退到系统 PATH
            ...
```

3. **新增命令行参数**:
```python
parser.add_argument(
    "--platform",
    choices=["windows", "darwin", "linux"],
    help="目标平台 (默认为当前平台)"
)
```

4. **`build_sidecar()` 函数支持目标平台参数**:
```python
def build_sidecar(target_platform=None):
    # 根据目标平台获取对应的 ffmpeg
    ffmpeg_path = get_ffmpeg_path(target_platform)
    ...
```

### 2. `.gitignore`

**新增**:
```
# Python ffmpeg cache
src-python/.ffmpeg_cache/
```

### 3. `src-python/.gitignore`

**新增**:
```
# ffmpeg 缓存目录
.ffmpeg_cache/

# Python
__pycache__/
*.py[cod]
...
```

### 4. `README.md`

**更新 Windows 构建说明**:
- 添加自动 ffmpeg 打包说明
- 三种构建方式
- 链接到详细指南
- PyInstaller 限制说明

### 5. `CHANGELOG.md`

**新增条目** (2025-12-09):
- Added: 自动 ffmpeg 下载和打包功能
- Changed: 优化构建流程
- Fixed: 解决 Windows 打包需要手动安装 ffmpeg 的问题

## 🚀 使用方式

### 方式 1: 完全自动 (推荐)

```bash
cd src-python
pip install -r requirements.txt
python build_sidecar.py --platform windows
```

### 方式 2: 先下载，再构建

```bash
cd src-python
python download_ffmpeg.py --platform windows
python build_sidecar.py --platform windows
```

### 方式 3: 使用系统 ffmpeg

```bash
# 安装 ffmpeg
choco install ffmpeg  # Windows
brew install ffmpeg   # macOS

# 构建
cd src-python
python build_sidecar.py
```

## 📊 文件结构

```
src-python/
├── download_ffmpeg.py          # 新增: ffmpeg 下载模块
├── test_ffmpeg_download.py     # 新增: 测试脚本
├── build_sidecar.py            # 修改: 支持自动下载
├── .gitignore                  # 修改: 忽略缓存目录
└── .ffmpeg_cache/              # 新增: 缓存目录 (git 忽略)
    ├── windows/
    │   └── ffmpeg.exe          (~80 MB)
    ├── darwin/
    │   └── ffmpeg              (~100 MB)
    └── linux/
        └── ffmpeg              (~90 MB)
```

## 🔍 工作原理

### 构建时

```
┌─────────────────────────┐
│ python build_sidecar.py │
└───────────┬─────────────┘
            │
            ▼
┌─────────────────────────┐
│  get_ffmpeg_path()      │
│  检查缓存               │
└───────────┬─────────────┘
            │
            ▼
    ┌───────┴───────┐
    │ 缓存存在?      │
    └───┬───────┬───┘
        │ 是    │ 否
        │       │
        │       ▼
        │   ┌─────────────────┐
        │   │ download_ffmpeg │
        │   │ 下载并缓存      │
        │   └────────┬────────┘
        │            │
        ▼            ▼
    ┌──────────────────────┐
    │ PyInstaller          │
    │ --add-binary=ffmpeg  │
    └──────────────────────┘
```

### 运行时

```
┌──────────────────────┐
│ Python Sidecar 启动  │
└─────────┬────────────┘
          │
          ▼
┌──────────────────────┐
│ VideoDownloader      │
│ _find_ffmpeg()       │
└─────────┬────────────┘
          │
          ▼
  ┌───────┴───────┐
  │ 检查位置       │
  └───┬───────┬───┘
      │       │
      │ sys._MEIPASS (PyInstaller 临时目录)
      │       │
      ▼       ▼
  ┌─────┐ ┌─────┐
  │找到 │ │系统 │
  │     │ │PATH │
  └─────┘ └─────┘
```

## ✅ 优势

### 对用户
- ✅ 无需手动安装 ffmpeg
- ✅ 零配置，开箱即用
- ✅ 避免安装错误
- ✅ 更好的用户体验

### 对开发者
- ✅ 简化构建流程
- ✅ 减少文档说明
- ✅ 降低支持成本
- ✅ 缓存加快后续构建

### 对部署
- ✅ CI/CD 更简单
- ✅ GitHub Actions 自动化
- ✅ 跨平台一致性
- ✅ 可重现的构建

## ⚠️ 限制和注意事项

### PyInstaller 跨平台限制

虽然可以下载任何平台的 ffmpeg，但:
- **Windows .exe 必须在 Windows 上构建**
- **macOS 应用必须在 macOS 上构建**
- **Linux 应用必须在 Linux 上构建**

这是 PyInstaller 的限制，不是脚本限制。

### 推荐方案

使用 GitHub Actions 多平台构建（已配置）:
```bash
git tag v1.0.0
git push origin v1.0.0
```

### macOS 特殊处理

macOS 不从网络下载 ffmpeg，而是：
1. 使用系统安装的 ffmpeg (`brew install ffmpeg`)
2. 复制到缓存目录
3. 打包到应用中

原因: macOS 版本的 ffmpeg 最好通过 Homebrew 安装，以确保签名和权限正确。

## 📝 测试结果

运行 `python test_ffmpeg_download.py`:

```
============================================================
VideoNote - ffmpeg 自动下载功能测试
============================================================

============================================================
测试 1: 获取当前平台的 ffmpeg
============================================================
✅ 成功! ffmpeg 路径: /path/to/.ffmpeg_cache/darwin/ffmpeg
✅ 文件存在，大小: 102.45 MB

============================================================
测试 2: 下载 Windows 平台的 ffmpeg
============================================================
正在下载: https://www.gyan.dev/ffmpeg/builds/...
进度: 100.0% (85234567/85234567 字节)
下载完成!
正在解压: /path/to/ffmpeg.zip
解压完成!
找到 ffmpeg: /path/to/extracted/ffmpeg.exe
✅ 成功! Windows ffmpeg 路径: /path/to/.ffmpeg_cache/windows/ffmpeg.exe
✅ 文件存在，大小: 81.23 MB
✅ 正确的 Windows 可执行文件格式

============================================================
缓存状态
============================================================
平台: darwin
  ✅ ffmpeg (102.45 MB)
平台: windows
  ✅ ffmpeg.exe (81.23 MB)

============================================================
测试完成!
============================================================
```

## 🎉 总结

现在构建 Windows 应用就像这样简单:

```bash
cd src-python
pip install -r requirements.txt
python build_sidecar.py --platform windows
cd ..
npm run tauri:build
```

**不需要**:
- ❌ 手动下载 ffmpeg
- ❌ 配置环境变量
- ❌ 安装到系统
- ❌ 担心版本问题

**只需要**:
- ✅ 一条命令
- ✅ 自动完成
- ✅ 开箱即用

---

## 📚 相关文档

- [WINDOWS_BUILD_GUIDE.md](./WINDOWS_BUILD_GUIDE.md) - Windows 构建详细指南
- [QUICK_START_CN.md](./QUICK_START_CN.md) - 快速开始指南
- [WINDOWS_DEPLOYMENT.md](./WINDOWS_DEPLOYMENT.md) - Windows 部署指南
- [CHANGELOG.md](./CHANGELOG.md) - 变更日志

---

**功能状态**: ✅ 已完成并测试  
**最后更新**: 2025-12-09




